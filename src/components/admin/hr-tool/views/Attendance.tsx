...
export default function Attendance() {
  ...
  async function punchIn() {
    ...
    await persistPunch({ emp: me.name, date: new Date().toISOString().slice(0, 10), inTime: time, inMinutes: minutes, outTime: existing?.outTime || null });
  ...
  async function punchOut() {
    ...
    const time = nowTimeStr();
    await persistPunch({ emp: me.name, date: new Date().toISOString().slice(0, 10), inTime: existing?.inTime || null, inMinutes: existing?.inMinutes ?? null, outTime: time });
    await syncAttendanceRecord(me.name);
  ...
  async function submitRegularization() {
    ...
    await persistRegularizations([{ id: 'R-' + Date.now(), emp: state.currentUser.name, date: regDate, reason, stage, status: 'pending', rmRemarks: '', hrRemarks: '' }, ...state.regularizations]);
    setRegOpen(false);
  }
  ...
  return (
    <>
      ...
      <section className="block">
        <div className="block-head"><h2>Regularization requests</h2>{isEmployeeOnly && <button className="btn sm" onClick={() => { setRegDate(todayStr()); setRegReason(REG_REASONS[0]); setRegReasonOther(''); setRegOpen(true); }}>+ Submit request</button>}</div>
        ...
        <div className="card"><table><thead><tr><th>Employee</th><th>Date</th><th>Reason</th><th>Status</th><th style={{ textAlign: 'right' }}>Action</th></tr></thead>
          <tbody>
            {regRows.map((r) => (
              <tr key={r.id}><td>{r.emp}</td><td>{r.date}</td><td>{r.reason}</td><td><ApprovalBadge req={r} /></td>
                <td style={{ textAlign: 'right' }}><ApprovalCell req={r} onDecide={(level, decision, remarks) => decideReg(r.id, level, decision, remarks)} /></td>
              </tr>
            ))}
            {regRows.length === 0 && <tr><td colSpan={5}><div className="empty">Nothing here.</div></td></tr>}
          </tbody>
        </table></div>
      </section>

      {regOpen && (
        <ModalShell title="Submit regularization request" onClose={() => setRegOpen(false)} actions={[
