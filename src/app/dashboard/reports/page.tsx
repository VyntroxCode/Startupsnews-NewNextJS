...
export default function ReportsPage() {
    ...
    (async () => {
      ...
      try {
        ...
        const [reportsRes, sectionsRes] = await Promise.all([
          ...
          fetch('/api/report-sections').catch(() => null),
        ]);

        const reportsData = await reportsRes.json();
        if (!reportsData.success) return;
  ...
  const isPdf = selectedReport?.mimeType === 'application/pdf' || selectedReport?.fileUrl.toLowerCase().endsWith('.pdf');
  const isImage = Boolean(selectedReport?.mimeType?.startsWith('image/'));

  const activeSection = sections.find((s) => s.id === sectionFilter) ?? null;

  const sectionReports = sectionFilter === null
    ? []
    : reports.filter((r) => r.sectionId === sectionFilter);
      ...
      <div style={{ padding: isMobile ? '1rem' : '1.5rem', minHeight: '100vh', boxSizing: 'border-box' }}>
        <div style={{ background: '#fff', borderRadius: 12, padding: isMobile ? '1.25rem' : '1.75rem 2rem', border: '1px solid #e5e7eb', marginBottom: '1.5rem' }}>
          ...
          <p style={{ fontSize: 15, color: '#4b5563', margin: 0 }}>
            Select a section from the sidebar to browse reports.
          </p>
        </div>
    ...
    <div style={{ padding: isMobile ? '1rem' : '1.5rem', minHeight: '100vh', boxSizing: 'border-box', width: '100%' }}>
      ...
      {loading ? (
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: '4rem 2rem', textAlign: 'center', border: '1px dashed #cbd5e1' }}>
          <p style={{ fontSize: 20, fontWeight: 700, color: '#0f172a', margin: '0 0 6px' }}>No reports found</p>
          ...
          {filtered.map((report) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div style={{ position: 'relative', width: '100%', aspectRatio: isMobile ? '4 / 3' : '16 / 9.5', background: hasPreview ? '#f9fafb' : `linear-gradient(135deg, ${report.bg}, #ffffff)`, overflow: 'hidden' }}>
                    {hasPreview ? (
                      <>
                        <Image src={imagePreviewUrl!} alt="" fill sizes="360px" style={{ objectFit: 'cover', filter: 'blur(28px) saturate(1.5)', opacity: 0.6, transform: 'scale(1.2)' }} onError={() => markPreviewBroken(report.id)} />
                        <Image src={imagePreviewUrl!} alt={report.title} fill sizes="360px" style={{ objectFit: 'contain', zIndex: 1 }} onError={() => markPreviewBroken(report.id)} />
                      </>
                    ) : (
