...
export default function AdminReportEditPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!reportFile && !fileUrl && !report.file_url) {
    ...
    try {
      ...
      if (reportFile) {
        ...
        finalFileSize = reportFile.size; // Ensure this is number or null
        finalMimeType = reportFile.type;
      } else if (!reportFile && fileUrl === '') {
      ...
      const apiRes = await fetch(withAdminToken(`/api/admin/reports/${reportId}`), {
        method: 'PUT',
    ...
    return (
      <AdminErrorBoundary>
        <div className="admin-content-area p-6 text-center text-slate-500">
          Report not found.
          <Link href="/admin/reports" className="text-indigo-600 hover:text-indigo-800 text-sm block mt-4">
            ← Back to Reports
          </Link>
        </div>
      </AdminErrorBoundary>
    ...
    <AdminErrorBoundary>
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <Link href="/admin/reports" style={{ color: '#6366f1', textDecoration: 'none', fontSize: '0.875rem', display: 'inline-block', marginBottom: '1rem' }}>
              ← Back to Reports
            </Link>
            <h2 style={{ fontSize: '2.25rem', fontWeight: '700', margin: '0 0 0.5rem', color: '#0f172a', letterSpacing: '-0.02em' ,marginBottom: '1rem'}}>
              Edit Report: {report.title}
            </h2>
            <p style={{ color: '#64748b', fontSize: '1rem', margin: 0 }}>
              Modify details for this research report.
            </p>
          </div>
        </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required={!fileUrl && !report?.file_url}
                    ...
                    />
                    {reportFile ? (
                      <p style={{ marginTop: '0.5rem', fontSize: '0.75rem', color: '#64748b' }}>New file selected: {reportFile.name} ({formatBytes(reportFile.size)})</p>
                    ) : report?.file_url ? (
                      <p style={{ marginTop: '0.5rem', fontSize: '0.75rem', color: '#64748b' }}>Current file: {report.file_name} ({formatBytes(report.file_size)})</p>
                    ) : (
                    ...
                    <input
                      ...
                      required={!reportFile && !report?.file_url}
