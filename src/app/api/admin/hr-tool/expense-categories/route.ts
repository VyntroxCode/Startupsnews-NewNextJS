...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    await hrToolService.saveExpenseCategories(body);
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Error saving HR expense categories:', error);
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Failed to save expense categories' },
      { status: 400 }
    );
  }
}
