...
export async function POST(request: NextRequest) {
  ...
  // Extract token from multiple sources (headers, form data, etc.)
  let formToken: string | null = null;
  if (contentType.includes('multipart/form-data')) {
    try {
      const clonedReq = request.clone();
      const formData = await clonedReq.formData();
      const ft = formData.get('_token');
      if (typeof ft === 'string' && ft.trim()) {
        formToken = ft.trim();
      }
    } catch (err) {
      console.warn('[Upload] Failed to extract token from FormData:', err);
    }
  }

  // Authenticate using the extracted token as fallback
  const auth = await getAuthUser(request, formToken);
  if (!auth) {
    console.warn('[Upload] Authentication failed', {
      hasAuthHeader: !!authHeader,
  ...
  try {
    ...
    // Handle Binary Octet-Stream Upload (Bypasses WAF body inspection)
    if (contentType.includes('application/octet-stream')) {
      const bytes = await request.arrayBuffer();
      fileBuffer = Buffer.from(bytes);

      fileName = request.nextUrl.searchParams.get('filename') || 'upload.jpg';
      fileMimeType = request.nextUrl.searchParams.get('type') || 'image/jpeg';

      console.log(`[Upload] Processing octet-stream upload: ${fileName} (${fileBuffer.length} bytes)`);
    }
