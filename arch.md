...
| 101 | 2026-09-19 | minor | Home page (iPhone) — DelegationStrip vs iOS navbar-pinning rule specificity | `globals.css`'s iOS-Safari `@supports (-webkit-touch-callout: none)` block (shipped in `f307949`) sets `#mvp-main-nav-bot { top: 0 !important }` at (2,1,2), out-specifying `delegation-strip.css`'s nav offset `.sn-ds + #mvp-main-head-wrap #mvp-main-nav-bot` (2,1,0) — so on iPhone the fixed nav sat at 0 under the fixed strip (z 10000) and the strip covered the logo. `delegation-strip.css` nav-offset selector raised to `html body .sn-ds + #mvp-main-head-wrap.startupnews-nav #mvp-main-nav-bot` (2,2,2); comments in both files record the coupling. Invariant: the iOS block's nav selector must stay below (2,2,2). Also row 909: migration audit — 61/61 schema migrations present on prod; `widen-hr-rules-half-day-threshold-hours.sql` never applied (`hr_rules.half_day_threshold_hours` still `int(11)`), pending user decision. | — |
| 102 | 2026-09-19 | minor | Home-page delegation strip (`components/DelegationStrip.tsx`) — copy shortened, text size bumped | User (screenshot of the live fixed-top strip) asked to drop "Indian" from the running line and increase the text size a little. Copy: both the visible line and the visually-hidden screen-reader `<p>` changed from "12th Indian Startup Delegation to Dubai" to "12th Startup Delegation to Dubai" — no markup/structure change. Sizing in `delegation-strip.css`: `.sn-ds-text`/`.sn-ds-dot` font-size 13px → 14.5px on desktop, and inside the ≤767px block 10.5px → 11.5px; item gaps, sparkle size and the CTA pill untouched. Drafted pre-pull as row 101, renumbered to 102 after `git pull` of `72a5026` took that number for the nav-specificity fix. Not built (standing rule). | — |
| 103 | 2026-09-19 | minor | `/expand-north-star` — top delegation band and the Participation Charges heading now hold a single line on phones | User screenshot: "Participation Charges, ₹1.65L onwards" wrapped on a phone; asked it and the top "Startup Delegation to Dubai" band both be single-line on mobile. **Band** (`.ens-delegation-title`, `expand-north-star.css`): deleted the `@media (max-width: 559px)` block that switched it to two lines — it existed only because a longer, pre-2026-09-18 wording ("Indian Startup Delegation to Dubai") made the width-proportional cqi size (`calc(100cqi / 18)`, already used ≥560px) too small to read; with "Indian" gone the same ratio lands at ~19px on a 375px phone, so the fallback no longer earns its keep. `.ens-delegation-line` (in `EnsDelegationTitle.tsx`) is now an unstyled span, kept only so the class exists if the two halves ever need separating again. **Fee heading** (`.ens-fee-title.is-oneline`, same file): its ratio-based nowrap sizing (`min(56px, calc(var(--ens-line-width) / 16.4 / 1.04))`) was gated to `@media (min-width: 760px)` like the other `is-oneline` headings; below that it fell back to `clamp(28px, 4.4vw, 56px)`, whose 28px floor no longer fit the line. Removed the gate for this heading only — its ratio still reads fine at ~17-20px down to a 320px viewport — while `.ens-new-heading`/`.ens-themes-title`/`.ens-journey-title` keep theirs, since their ratios would go illegibly small on a phone. Verified on an isolated `next dev -p 3021` (PM2 untouched, removed after) with Playwright screenshots at 375px and 320px: both lines render whole, no overflow. tsc clean. Not built. | — |
| 104 | 2026-09-19 | minor | `/events` page — `groupByCountry`'s country-level ordering (`src/app/events/page.tsx`) | "Cohort" and "Online" are non-geographic labels that nonetheless become top-level keys of the `grouped` country map (`isCohort` sets `country = "Cohort"`; `resolveCountry` falls back to the region's own name, "Online", when no event carries a real country). The final sort previously only special-cased "India first", so these two interleaved alphabetically among real countries. Added a `TRAILING_COUNTRIES` set (`COHORT_PARTNERSHIP_TYPE`, `"Online"`); the comparator now checks trailing-membership before the India/alphabetical rule, so both always render after every real country (alphabetical between the two of them). Invariant: any future non-geographic top-level country key must be added to `TRAILING_COUNTRIES` to keep this ordering, the same way it must already be added to `NON_GEOGRAPHIC_REGIONS` (`region-country.utils.ts`) to avoid being treated as a city. | — |
| 105 | 2026-09-19 | medium | `/expand-north-star` — Angel Bay referral logo corrected, Indicorn Angels logo added, both linked out; "Indicornangels" added to the "Referred By" form list | User supplied two logo files into `public/images/expand-north-star/` and flagged that the live Angel Bay logo (`referral-angel-bay.jpg`) was actually the "Angel Bay by ANZCO Foods" meat-brand mark — a different company sharing the name, not this page's angel-investment referrer. Confirmed by downloading and viewing the live S3 object. Uploaded the two supplied files (`PutObjectCommand`, throwaway script deleted after running) to NEW keys — `referral-angel-bay-v2.jpg` and `referral-indicorn-angels.jpg` — rather than overwriting the old key, since the CDN caches each key for a year; both confirmed `200` after upload. `media.ts`'s `ENS_REFERRAL_LOGOS`: `angel-bay` repointed to the `-v2` key, `indicorn-angels` added. `referralPartnerLogos.ts`: `ReferralPartnerLogo` gains an optional `linkUrl`, set for `angel-bay` (`https://angelbay.com/`) and the new `indicorn-angels` (`https://indicornangels.com/`) row; the marquee-mapping's hardcoded `linkUrl: null` now reads `logo.linkUrl ?? null` — `PartnerLogoTile` already renders a `linkUrl` as an `<a target="_blank">`, so no shared-component change was needed. Every other logo in the strip still has no `linkUrl`. `modules/ens-travel-enquiries/domain/sources.ts`'s `REFERRED_BY_OPTIONS` gains `{ value: 'indicorn-angels', label: 'Indicornangels' }` — the form, its validation and the Sales Tracker admin screens all derive from this one array, so no other file needed a matching edit. Local files deleted from `public/images/expand-north-star/` once their S3 copies were confirmed live. `tsc --noEmit` clean on all touched files. Not built (standing rule). | v35 |
| 106 | 2026-09-19 | minor | `src/components/PartnerLogosMarquee.tsx` — clicking a linked logo (Angel Bay, Indicorn Angels on `/expand-north-star`; any future `/our-partners` admin logo with a link) never opened the link, on any row, dragged or not | User reported the two new logos from row 105 weren't clickable. Root cause found by instrumenting real click events in headless Chromium against the running site: `onPointerDown` called `row.setPointerCapture(e.pointerId)` unconditionally on every press, including a motionless click. Per the Pointer Events spec, an element holding pointer capture becomes the target of the subsequent `pointerup` **and the resulting `click` event** — confirmed live: `click.target` was `DIV.partners-marquee-row`, not the anchor, even with zero mouse movement between down and up. Since the click's target was no longer inside the `<a>`, the browser never ran its default link-navigation action — the existing `DRAG_SLOP_PX` click-suppression logic was never even reached, since propagation had already been retargeted away from the anchor before that handler could act. **Fix:** capture is now deferred from `onPointerDown` to `onPointerMove`, taken only once `pressMovement` crosses `DRAG_SLOP_PX` (i.e., once it's confirmed to be a real drag, not a click); `onPointerUp` releases via a tracked `capturedPointerId` instead of assuming capture was always taken. Verified on an isolated `next dev -p 3021` (PM2's `startupgpt-dev` on :3010 untouched throughout, port killed after): headless-Chromium click events now target the clicked `<img>`/`<a>` and `window.open` fires with the correct URL; a real 80px drag still slides the track (`translate3d` offset changed) and still opens no popup. `tsc --noEmit` + eslint clean. This is the same shared component `/our-partners` uses, so any admin-configured logo `linkUrl` there was equally broken and is fixed by the same change. Not built/restarted on the main process (standing rule) — the live `startupgpt-dev` process was independently rebuilt/restarted by an external process during this session (pm2 showed 12 restarts, most recent `next start` at 15:13:47), which is how row 105's logos and links were already visible live before this fix; this fix still needs its own build+restart to reach that process. | — |
| 107 | 2026-09-19 | minor | `src/components/delegation-strip.css` — banner-carousel clearance rule vs `globals.css` ID specificity, mobile-only fix | User (screenshot, iPhone width) reported the home-page banner-carousel sitting flush against the navbar with no gap; clarified the issue is mobile-only. Root cause: row 879's clearance rule, `body .sn-ds ~ .banner-carousel-container { margin-top: calc(72px + var(--sn-ds-h)) !important }` (specificity 0,2,1), never won the cascade against `globals.css`'s `#mvp-site-main .banner-carousel-container { margin-top: 72px !important }` (1,1,0) — an ID beats any number of classes, so the container's margin-top stayed a flat 72px on every viewport, 38-44px short of clearing the strip-plus-nav fixed header. On mobile the banner's own 20px top margin (28px on desktop) wasn't enough to visually hide that shortfall, so the carousel appeared glued to the navbar. Fix, scoped to the existing `@media (max-width: 767px)` block only (desktop's larger margin absorbs the same underlying gap and wasn't reported as broken): re-declared the rule with the site ID, `#mvp-site-main .sn-ds ~ .banner-carousel-container { margin-top: calc(72px + var(--sn-ds-h)) !important }` (1,2,0), which now wins. Verified on an isolated `next dev -p 3099` (PM2's `startupgpt-dev` on :3010 untouched, killed after): mobile `.banner-carousel-container` margin-top now 110px (72+38) with a 26px visible gap below the nav; desktop unchanged at 72px. Invariant: any future rule targeting `.banner-carousel-container`'s or `.sn-ds`-dependent siblings' box model must include an ID selector (`#mvp-site-main` or `html body #mvp-main-head-wrap`-style) to beat globals.css's ID-anchored banner rules — the same pattern rows 101/103 already established for the nav-top and body-wrap-padding overrides. Not built/restarted on the main process (standing rule). | — |
| 108 | 2026-09-20 | minor | `/expand-north-star` event bar (`EnsNav.tsx` / `expand-north-star.css`, `@media (max-width: 760px)`) — Participate Now button was wrapping onto its own third line on a phone | User's screenshot (iPhone, ~390px CSS width): "Launchpad Middle East · Dubai Konnect" sat on one line under the logo lockup as intended, but the "Participate Now" button had wrapped onto a further line below instead of sharing that same line. Root cause, measured in headless Chromium at several phone widths: the mobile override `.ens-nav .ens-nav-launchpad { flex-direction: row; gap: 0.35em }` (undated, pre-existing) put "Launchpad" and "Middle East" side by side, ~205px wide at 390px — combined with the Konnect logo, its margin and the button, the wrapped row's total content width (≈345px) exceeded the row's available width (≈354px at 390, and progressively less at every narrower phone), so the flex row (all items `flex: none`, no shrink) pushed the last item — the button — onto a new line once the sum stopped fitting. **Fix:** (1) `.ens-nav .ens-nav-launchpad`'s mobile override now only touches `font-size` (floor lowered 12px → 10.5px) — direction is inherited from the desktop rule's `flex-direction: column`, so "Launchpad"/"Middle East" stack instead of sitting side by side, cutting that element's width from ~205px to ~100px at 390px. (2) Reclaimed further width so the fit holds down to a 320px phone, not just 390px: `.ens-nav-inner`'s mobile `column-gap` 10px → 6px, `.ens-nav-konnect-img` height 30px → 24px, `.ens-nav-konnect`'s `margin-right` 12px → 6px, `.ens-page .ens-nav-cta`'s mobile `padding` `0.6em 1em` → `0.55em 0.85em`, `font-size` 10.5px → 9px, `letter-spacing` 0.04em → 0.03em. Verified on an isolated `next dev -p 3021` (PM2's `startupgpt-dev` on :3010 untouched, killed after) at 320/360/375/390/414/428/600/760/761/900px: Launchpad, the Konnect logo and the button share one line (top-aligned within 5px of each other) at every phone width from 320 up through 760; no horizontal overflow at any width; the ≥761px desktop/tablet single-row layout (untouched rules) is visually unchanged. CSS-only; `tsc --noEmit` clean. Not built/restarted on the main process (standing rule). | — |
| 109 | 2026-09-20 | minor | `/expand-north-star` — Dubai Konnect logo too close to "Launchpad Middle East" on a phone (mobile nav, row 108's fix); nav scroll-progress line was pink-to-purple, now solid pink | User's screenshot of the mobile event bar, right after row 108's fix, showed the Konnect logo sitting almost flush against the Launchpad text, and separately asked the reading-progress line along the bar's bottom edge to be pink only, not purple — purple isn't in this page's palette. **Gap:** measured the actual gaps in headless Chromium — the divider-to-Konnect gap was a flat 6px (the mobile `column-gap` from row 108) at every phone width, while the gap between the Konnect logo and the "Participate Now" button ranged 21-114px, entirely absorbed by the button's `margin-left: auto`. Rather than widen `column-gap` (which would have also widened the already-generous button gap and risked re-wrapping it at 320px, per row 108's tight margins), added `margin-left: 8px` to `.ens-nav-konnect` specifically, taking the visible gap from ~6px to ~14px. 8px was chosen empirically as the largest value that still keeps the button on the same line at a 320px phone — binary-searched in headless Chromium; 9px was the first value to wrap it back onto a third line. **Progress line:** `.ens-nav-progress`'s `background: linear-gradient(90deg, var(--ens-pink), #9B4DFF)` → `background: var(--ens-pink)` (solid, no gradient) — `#9B4DFF` was a one-off purple with no page variable, unrelated to `--ens-pink`/`--ens-pink-dark`. The Participation Fee section's own progress-style accent (`expand-north-star.css` ~line 1512) uses the same pink→purple gradient but was not reported broken and was left untouched. Verified on an isolated `next dev -p 3021` (PM2's `startupgpt-dev` on :3010 untouched, killed after): gap ≈14px and the row still single-line, no overflow, at 320/360/375/390/414/428/760/900px; `.ens-nav-progress`'s computed `background-color` is `rgb(228, 21, 124)` (`--ens-pink`) with no `background-image`. CSS-only; `tsc --noEmit` clean. Not built/restarted on the main process (standing rule). | — |
| 110 | 2026-09-20 | minor | Home page — banner-carousel clearance rule vs `globals.css` ID specificity, now fixed at every width (supersedes row 107's mobile-only patch) | User's screenshot (desktop browser window) showed the home-page banner-carousel's top edge tucked behind the fixed strip+nav — the same overlap row 107 fixed on mobile only, on the stated assumption that "desktop's overlap is small enough to be visually absorbed by the banner's own top padding and hasn't been reported as an issue." That assumption was wrong: the root cause — `delegation-strip.css`'s `body .sn-ds ~ .banner-carousel-container { margin-top: calc(72px + var(--sn-ds-h)) !important }` (0,2,1) losing the cascade to `globals.css`'s `#mvp-site-main .banner-carousel-container { margin-top: 72px !important }` (1,1,0), an ID beating any number of classes — applies at every viewport width equally; only the visible severity differs (44px of the banner's top hidden on desktop, matching the screenshot, vs the mobile symptom row 107 described). **Fix:** added `#mvp-site-main` to the base (always-on) selector — `#mvp-site-main .sn-ds ~ .banner-carousel-container` (1,2,0) — so it now wins the cascade at every width without a separate mobile-only patch; row 107's now-redundant `@media (max-width: 767px)` duplicate of the same override was deleted (it would have been byte-identical to the base rule). The `#mvp-main-body-wrap` padding-top pair was left alone — its globals.css ID-bearing counterparts already resolve to the same 6px value whenever a banner is present, so there was nothing visibly wrong there. Verified on an isolated `next dev -p 3021` (PM2's `startupgpt-dev` on :3010 untouched, killed after): `.banner-carousel-container`'s computed `margin-top` is 116px (72+44) at 1440/1024/768px and 110px (72+38) at 767/390/375px, both correct, with the banner's top edge now clear of the fixed strip+nav at every size; a page with no delegation strip (`/news`) is unaffected, still 72px. CSS-only; `tsc --noEmit` clean. Not built/restarted on the main process (standing rule). | — |
| 111 | 2026-09-20 | minor | Desktop only — extra gap between fixed navbar and banner-carousel, both with and without the delegation strip | User asked for more margin-top between navbar and banner-carousel in desktop view. First attempt added a bare `.banner-carousel-container { margin-top: 92px !important }` inside `globals.css`'s `@media (min-width: 1024px)` block (0,1,0) — caught before commit by re-reading row 110's history: `.banner-carousel-container` is nested under `#mvp-site-main` (`ConditionalLayout.tsx`), so the base rule effectively applies at (1,1,0) and would have silently beaten the new bare-class rule regardless of `!important`, the exact landmine row 110 describes. **Fix, two selectors to cover both cases:** (1) `globals.css`'s desktop block now uses `#mvp-site-main .banner-carousel-container { margin-top: 92px !important }` (1,1,0) — ties the base rule's specificity and wins by appearing later in the cascade; applies on strip-less pages (e.g. `/news`), 72px → 92px. (2) `delegation-strip.css` gained a matching `@media (min-width: 1024px)` block reusing `#mvp-site-main .sn-ds ~ .banner-carousel-container` (1,2,0) with `margin-top: calc(72px + var(--sn-ds-h) + 20px) !important` — applies on the home page (strip present), 116px → 136px. Verified with a temporary Playwright install against an isolated `next dev -p 3021` (PM2's `startupgpt-dev` on :3010 untouched, killed after, Playwright install removed after): computed `margin-top` is 136px (home) / 92px (`/news`) at 1440px and 1024px, and unchanged at 110px (home) / 72px (`/news`) at 767px and below — mobile/tablet untouched. CSS-only; `tsc --noEmit` clean. Not built/restarted on the main process (standing rule). | — |
